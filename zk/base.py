# -*- coding: utf-8 -*-
import sys  # from builtins import str
...
class ZK_helper(object):
    ...
    def test_ping(self):
        ...
        ping_str = "-n 1" if is_windows else "-c 1 -W 5"

        args = "ping " + " " + ping_str + " " + self.ip
        need_sh = False if is_windows else True
...
class ZK(object):
    ...
    def __test_tcp_top(self, packet):
        ...
        if self.verbose:
            print("header tcp top {}".format(tcp_header))
    ...
    def get_user_template(self, uid, temp_id=0, user_id=''):
        ...
        """
        if self.verbose:
            print ("starting get_user_template")
        if not uid:
            users = self.get_users()
            users = list(filter(lambda x: x.user_id == str(user_id), users))
            if not users:
        ...
        for _retries in range(3):
            if self.verbose:
                print ("try {} get_user_template".format(_retries))
        ...
        if cmd_response.get('code') == const.CMD_PREPARE_DATA:
            ...
            if self.tcp:
                ...
                if self.verbose:
                    print ("received {}, size {} rec {}".format(
                        recieved,
                        size,
                        data_recv.encode('hex')))  # todo python3
                ...
                if recieved >= (bytes + 32):  # complete
                    if response == const.CMD_DATA:
                        ...
                        if resp[-6:] == b'\x00\x00\x00\x00\x00\x00':  # padding? bug?
                            if self.verbose:
                                print ("cutting from", len(resp))
                            resp = resp[:-6]
                        if self.verbose:
                            print ("resp len1", len(resp))
    ...
    def enroll_user(self, uid=0, temp_id=0, user_id=''):
        ...
        if self.tcp:
            ...
            command_string = pack('<Ib', int(user_id), temp_id)  # self.cancel_capture()
        cmd_response = self.__send_command(command, command_string)
        if not cmd_response.get('status'):
        ...
        while not self.end_live_capture:
            try:
                ...
                if len(data) == 12:  # class 1 attendance  # TODO: RETEST ZK6
                    ...
                    if self.verbose:
                        print (codecs.encode(data, 'hex')), len(data)
    ...
    def __receive_chunk(self):
        ...
        if self.__response == const.CMD_DATA:  # less than 1024!!!
            ...
            if self.verbose:
                print ("receive chunk:data size is", size)
            ...
            if self.tcp:
                ...
                if recieved >= (size + 32):  # complete with ACK_OK included
                    if response == const.CMD_DATA:
                        ...
                        if self.verbose:
                            print ("resp complete len", len(resp))
                        return resp
                    else:
                        if self.verbose:
                            print("broken packet!!! {}".format(response))
                        return None  # broken
                else:  # incomplete
                    if self.verbose:
                        print ("try incomplete (actual valid {})".format(recieved-16))
                    ...
                    if size < 0:  # broken ack header?
                        ...
                        if self.verbose:
                            print ("broken", (broken_header).encode('hex'))  # TODO python3
                    while size > 0:  # jic
                        if self.verbose:
            ...
            while True:  # limitado por respuesta no por tamaño
                ...
                if response == const.CMD_DATA:
                    ...
                    break
                if self.verbose:
                    print ("still needs %s" % size)
            return b''.join(data)
        else:
            if self.verbose:
                print ("invalid response %s" % self.__response)
            return None  # ("can't get user template")
